...
/datum/hud/proc/ButtonNumberToScreenCoords(number, supportedcolumns = AB_MAX_COLUMNS) // TODO : Make this zero-indexed for readabilty
...
/datum/hud/proc/SetButtonCoords(atom/movable/screen/button,number, supportedcolumns = AB_MAX_COLUMNS)
	var/row = round((number-1)/supportedcolumns)
	var/col = ((number - 1)%(supportedcolumns)) + 1
