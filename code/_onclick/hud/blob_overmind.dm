...
/atom/movable/screen/blob/resource_blob/Click()
	if(isovermind(usr))
		var/mob/camera/blob/B = usr
		B.createSpecial(40, /obj/structure/blob/special/resource, BLOB_RESOURCE_MIN_DISTANCE, TRUE)
...
/atom/movable/screen/blob/node_blob/Click()
	if(isovermind(usr))
		var/mob/camera/blob/B = usr
		B.createSpecial(50, /obj/structure/blob/special/node, BLOB_NODE_MIN_DISTANCE, FALSE)
...
/atom/movable/screen/blob/factory_blob/Click()
	if(isovermind(usr))
		var/mob/camera/blob/B = usr
		B.createSpecial(60, /obj/structure/blob/special/factory, BLOB_FACTORY_MIN_DISTANCE, TRUE)
...
/atom/movable/screen/blob/readapt_strain/MouseEntered(location,control,params)
	if(hud?.mymob && isovermind(hud.mymob))
		...
		if(B.free_strain_rerolls)
			...
			name = "[initial(name)] ([40])"
			desc = "Allows you to choose a new strain from [40] random choices for [BLOB_POWER_REROLL_COST] resources."
	..()
