...
/datum/reagent
	...
	// Fermichem vars //
	/// How pure a chemical is from 0 - 1.
	var/purity = 1
	var/cached_purity = 1
	/// Should be the creation location!
	var/turf/loc = null
	/// pH of the specific reagent, used for calculating the sum pH of a holder.
	var/pH = 7
	/// What chemical is metabolised with an inpure reaction
	var/impure_chem
	/// If the impurity is below 0.5, replace ALL of the chem with inverse_chemupon metabolising
	var/inverse_chem_val = 0
	/// What chem is metabolised when purity is below inverse_chem_val, this shouldn't be made, but if it does, well, I guess I'll know about it.
	var/inverse_chem
	/// See fermi/readme.dm REAGENT_DEAD_PROCESS, REAGENT_DONOTSPLIT, REAGENT_ONLYINVERSE, REAGENT_ONMOBMERGE, REAGENT_INVISIBLE, REAGENT_FORCEONNEW, REAGENT_SNEAKYNAME
	var/chemical_flags
	/// How much does it sell for in cargo?
	var/value = REAGENT_VALUE_NONE
