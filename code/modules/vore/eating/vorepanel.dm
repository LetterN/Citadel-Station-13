	...
	switch(action)
		...
		if("newbelly")
			...
			if(failure_msg) //Something went wrong.
				to_chat(usr, span_warning(failure_msg))
				return TRUE
		...
		if("saveprefs")
			if(!host.copy_to_prefs_vr())
				to_chat(usr, "<span class='warning'>Belly Preferences not saved!</span>")
		...
		if("reloadprefs")
			...
			if(!host.copy_from_prefs_vr())
				to_chat(usr, span_warning("ERROR: Virgo-specific preferences failed to apply!"))
			else
		...
		if("setflavor")
			...
			if(length(new_flavor) > FLAVOR_MAX)
				to_chat(usr, span_info("Entered flavor/taste text too long. [FLAVOR_MAX] character limit."))
				return FALSE
		...
		if("setsmell")
			...
			if(length(new_smell) > FLAVOR_MAX)
				to_chat(usr, span_info("Entered perfume/smell text too long. [FLAVOR_MAX] character limit."))
				return FALSE
...
/datum/vore_look/proc/set_attr(mob/user, params)
	if(!host.vore_selected)
		to_chat(usr, span_info("No belly selected to modify."))
		return FALSE
	...
	switch(attr)
		if("b_name")
			...
			if(failure_msg) //Something went wrong.
				to_chat(user, span_warning(failure_msg))
				return FALSE
		...
		if("b_desc")
			...
			if(new_desc)
				...
				if(length(new_desc) > BELLIES_DESC_MAX)
					to_chat(user, span_info("Entered belly desc too long. [BELLIES_DESC_MAX] character limit."))
					return FALSE
		...
		if("b_verb")
			...
			if(length(new_verb) > BELLIES_NAME_MAX || length(new_verb) < BELLIES_NAME_MIN)
				to_chat(usr, span_info("Entered verb length invalid (must be longer than [BELLIES_NAME_MIN], no longer than [BELLIES_NAME_MAX])."))
				return FALSE
		...
		if("b_escapable")
			if(host.vore_selected.escapable == 0) //Possibly escapable and special interactions.
				...
				to_chat(usr, span_warning("Something went wrong. Your stomach will now not have special interactions. Press the button enable them again and tell a dev."))
				host.vore_selected.escapable = 0
			. = TRUE
		if("b_escapechance")
